...
import meldexun.lagspikeprofiler.asm.tweaker.LagSpikeProfilerTweaker;
...
@IFMLLoadingPlugin.TransformerExclusions({ "meldexun.lagspikeprofiler.asm", "meldexun.asmutil2" })
public class LagSpikeProfilerPlugin implements IFMLLoadingPlugin {

	@SuppressWarnings("unchecked")
	public LagSpikeProfilerPlugin() {
		try {
			if (((List<ITweaker>) Launch.blackboard.get("Tweaks")).stream().noneMatch(FMLInjectionAndSortingTweaker.class::isInstance)) {
				((List<String>) Launch.blackboard.get("TweakClasses")).add(LagSpikeProfilerTweaker.class.getName());
			} else {
				((List<ITweaker>) Launch.blackboard.get("Tweaks")).add(new LagSpikeProfilerTweaker());
			}
			Field _tweakSorting = CoreModManager.class.getDeclaredField("tweakSorting");
			_tweakSorting.setAccessible(true);
			((Map<String, Integer>) _tweakSorting.get(null)).put(LagSpikeProfilerTweaker.class.getName(), 1001);
